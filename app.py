...
# Define fixed-size lists (deque) to store the last 20 data points for RAM,
# CPU, Disk usage, and time. This is bounded deque with auto pop at the opposite end
history = {
    "ram": deque(maxlen=20),
    "cpu": deque(maxlen=20),
    "disk": deque(maxlen=20),
    "time": deque(maxlen=20),  # x-axis
}
...
def get_system_stats():
    try:
        ...
        return {"RAM Usage (%)": ram, "CPU Usage (%)": cpu, "Disk Usage (%)": disk}
...
# - one: Show ALL metrics on 'one' chart
# - multiple: Show each metric on it's own Chart
...
if mode == "one":
    ...
    @app.callback(
        Output("combined-graph", "figure"), [Input("interval-component", "n_intervals")]
    )
